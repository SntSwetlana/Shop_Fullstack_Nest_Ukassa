...
export const menuStyles = (
  ...
  width: 'calc(100% + 40px)', // 40px for input button
  minHeight: 30, //for spinner component
})
...
export const optionStyles = (
  ...
  const coverHoverForLightMode = state.isSelected
    ? state.isSelected
      ? '#f2f2f2'
      : '#9e9e9e'
    : state.isSelected
  ...
  const coverHoverForDarkMode = state.isSelected
    ? state.isSelected
      ? '#9e9e9e'
      : '#f2f2f2'
    : state.isSelected
  ...
  return {
    ...
    '&:hover': {
      ...
      color: theme === 'dark' ? coverHoverForDarkMode : coverHoverForLightMode,
    },
    backgroundColor:
     theme === 'dark'
       ? state.isSelected
