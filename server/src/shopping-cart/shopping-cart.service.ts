...
export class ShoppingCartService {
  ...
  async updateCount(
    ...
    await this.shoppingCartModel.update({ count }, { where: { partId } }); //обновили count в элементе корзины
  ...
  async updateTotalPrice(
    ...
    await this.shoppingCartModel.update({ total_price }, { where: { partId } }); //обновили count в элементе корзины
    const part = await this.shoppingCartModel.findOne({ where: { partId } });
